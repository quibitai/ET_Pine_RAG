...
export async function processFileForRag({
  ...
  try {
    ...
    console.error('[RAG Processor] Failed to fetch document details:', error);
    await updateFileRagStatus({ 
      id: documentId, 
      processingStatus: 'failed', 
      statusMessage: 'Failed to fetch document details' 
    });
    return false;
  }

  if (!docDetails) {
    console.error(`[RAG Processor] No document found with ID: ${documentId}`);
    await updateFileRagStatus({ 
      id: documentId, 
      processingStatus: 'failed', 
  ...
  try {
    ...
    console.error('[RAG Processor] Failed to extract text:', error);
    await updateFileRagStatus({ 
      id: documentId, 
      processingStatus: 'failed', 
      statusMessage: 'Failed to extract text from file' 
    });
    return false;
  }
